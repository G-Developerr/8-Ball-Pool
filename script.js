...
class Cue {
    ...
    draw() {
        ...
        const endX = cueBall.x + Math.cos(this.angle) * 600;
        const endY = cueBall.y + Math.sin(this.angle) * 600;

        // 1. Γραμμή καθοδήγησης (guide line) με διακεκομμένη γραμμή
        ctx.beginPath();
        ...
        // 2. ΣΚΙΑ ΣΤΕΚΑΣ (πιο μαλακή και μεγαλύτερη)
        ctx.beginPath();
        ...
        // 3. ΚΥΡΙΟ ΣΩΜΑ ΣΤΕΚΑΣ (με πιο έντονα gradients και ανταύγειες)
        const cueLength = 220;
        const tipStart = -30 - this.pullDistance;
        const buttEnd = -250 - this.pullDistance;

        // Βασικό gradient (ξύλο + μεταλλικές λεπτομέρειες)
        ...
        // Σχεδίαση κυρίως σώματος (πιο παχύ και με λεπτομέρειες)
        ctx.beginPath();
        ...
        // 4. ΜΥΤΗ ΣΤΕΚΑΣ (πιο λευκή και γυαλιστερή)
        ...
        // Ανταύγεια στην μύτη (highlight)
        ctx.beginPath();
        ctx.arc(tipStart + 12, -2, 2, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
        ctx.fill();

        // 5. ΔΙΑΚΟΣΜΗΤΙΚΕΣ ΛΩΡΙΔΕΣ (όπως πραγματικές στέκες)
...
function checkFoul() {
    ...
    if (!isFoul && player1Type && player2Type && firstHitBall) {
        ...
        // Αν χτυπήθηκε πρώτα η 8-ball χωρίς να έχουν βάλει όλες τις δικές τους
...
function drawTable() {
    // Πράσινο τραπέζι με ντεγκραντέ για πιο ρεαλιστική εμφάνιση
    const texture = ctx.createLinearGradient(0, 0, 0, canvas.height);
    texture.addColorStop(0, "#0f5c2b");
    texture.addColorStop(1, "#0a4d25");
    ctx.fillStyle = texture;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Ξύλινο περίγραμμα πιο "ρεαλιστικό"
    ctx.strokeStyle = "#4e342e";
    ctx.lineWidth = 20;
    ctx.strokeRect(0, 0, canvas.width, canvas.height);

    // Κεντρική διακεκομμένη γραμμή (σεμι-διάφανη)
    ...
    // Τελείες καθοδήγησης (πιο διακριτικές)
    const dots = [
